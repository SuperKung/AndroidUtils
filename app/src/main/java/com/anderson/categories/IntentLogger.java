...
//private IntentLogger() {}
    ...
    public static void dump(Intent intent) {
        if (intent == null) {
            AndroidUtils.log.v("no intent found");
            return;
        }
        Bundle extras = intent.getExtras();
        AndroidUtils.log.v("Intent[@" + Integer.toHexString(intent.hashCode()) + "] content:");
        AndroidUtils.log.v("Action   : " + intent.getAction());
        AndroidUtils.log.v("Category : " + intent.getCategories());
        AndroidUtils.log.v("Data     : " + intent.getDataString());
        AndroidUtils.log.v("Component: " + intent.getComponent().getPackageName() + "/" + intent.getComponent().getClassName());
        dumpFlags(intent.getFlags());
        AndroidUtils.log.v("HasExtras: " + (extras != null && !extras.isEmpty()));
        dumpExtras(extras);
    }
    ...
    public static void dumpFlags(int flags) {
        AndroidUtils.log.v("Flags    : " + Integer.toBinaryString(flags));
        for (int flag : FLAGS.keySet()) {
            if ((flag & flags) != 0) {
                AndroidUtils.log.v("Flag     : " + FLAGS.get(flag));
            }
        }
    }
    ...
    public static void dumpExtras(Bundle extras) {
        ...
        for (String key : extras.keySet()) {
            AndroidUtils.log.v("Extra[" + key + "] :" + String.valueOf(extras.get(key)));
        }
    }
}
